...
while not done:
    try: # This method tries for a valid input
        ...
        print("That is not a valid floating number. \n")
print(f"The number that you entered is {num}.")'''
...
# To create a funtion, start with the word def
'''
def test_int_num():
    done = False
    while not done:
...
# Code that tests whether a vali input is given (v1.4)
# Use the function parameters to mke my code 
# more pythonic
...
def valid_num(question,low,high):
    ...
    while True:
        try:
            ...
            #if responce >= 0 and responce <=10:
            if low <= response <= high:
                break # This stops th loop
    ...
    return response # This makes the response avaiabe to be used
...
num_3 = valid_num("Enter a number between 40 and 67 ",40,67 )
print(f"You entered {num_2}\n")

# Multiply the result of num_1, num_2, and num_3
multiply = num_1 * num_2 * num_3
print(f"Your threenumbers multiplied together are qual to {multiply}\n")

# Adding the numbers
sum = num_1 + num_2 + num_3
print(f"The total of {num_1}, {num_2}, {num_3}")
